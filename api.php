...
$webstoreIdent = TEBEX_PUBLIC_KEY; // Enter either your token (Not your API Key, Just the token.) or you can attach it from another file when making the request.
 ...
 *  @return array decoded from Tebex's API
 */
function CreateMinecraftBasket(string $username, string $complete_url, string $cancel_url, array $custom, bool $complete_auto_redirect, string $ip_address = "")
{
  global $webstoreIdent;
  $requestData = array(
    "username" => $username,
  ...
  // Si se proporciona una IP, agregala al arreglo de datos
  if (!empty($ip_address)) {
 ...
 *  @return array decoded from Tebex's API
 */
function AddPackage(string $basketIdent, string $package_id, int $quantity = 1, array $custom = array(), string $type = "single", array $variable_data = array())
{
  return Request(
    "POST",
    "baskets",
